...
def get_ohlc_data(pair: str, timeframe: str) -> pd.DataFrame:
    ...
    if pair in FOREX_PAIRS:
        return _fetch_forex_yf(pair, timeframe)
    if pair in CRYPTO_PAIRS:
        try:
            return _fetch_crypto_binance(pair, timeframe)
        except Exception:
            return _fetch_crypto_yf_fallback(pair, timeframe)
    # Synthetic indices
    return _synthetic_ohlc(pair, timeframe)
