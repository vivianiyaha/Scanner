...
def generate_ohlc_data(pair, timeframe, n=100):
    ...
    volatility = base * 0.0015 * (4 if any(s in pair for s in ["Boom","Crash","Volatility"]) else 1)
    ...
    timestamps = [end_time - timedelta(minutes=minutes*i) for i in range(n-1, -1, -1)]
    ...
    for i, (ts, close) in enumerate(zip(timestamps, closes)):
        ...
        data.append({"timestamp": ts, "open": open_, "high": high, "low": low, "close": close,
...
def generate_signal(pair, timeframe="H1", h4_trend=None):
    ...
    df = generate_ohlc_data(pair, timeframe)
...
def build_chart(sig):
    ...
    # EMAs
    fig.add_trace(go.Scatter(x=df["timestamp"], y=df["ema20"],  line=dict(color="#FFD700", width=1), name="EMA 20"), row=1, col=1)
    fig.add_trace(go.Scatter(x=df["timestamp"], y=df["ema50"],  line=dict(color="#FF8C00", width=1.5), name="EMA 50"), row=1, col=1)
    fig.add_trace(go.Scatter(x=df["timestamp"], y=df["ema200"], line=dict(color="#4169E1", width=2), name="EMA 200"), row=1, col=1)
...
def analyze_chart_image(image):
    ...
    # Simulated analysis (real deployment would use vision AI)
    detected_pair  = random.choice(FOREX_PAIRS)
    detected_tf    = random.choice(["H1", "H4", "M15"])
    simulated_sig  = generate_signal(detected_pair, detected_tf)

    notes = [
        f"Chart dimensions: {w}×{h}px — sufficient for analysis.",
        f"Pair auto-detected: {detected_pair} (pattern similarity match)",
        f"Timeframe estimated: {detected_tf} (candle interval analysis)",
        "Market structure extracted from price action patterns.",
        "Key levels identified using swing point detection.",
    ]
    return simulated_sig, notes
    ...
    with tabs[0]:
        if not st.session_state.scan_results:
            ...
            for r in results:
                ...
                score_str = f"{r['score']}%"
                rows.append({
                    "": sig_color, "Pair": r["pair"], "TF": r["timeframe"],
                    "Signal": r["signal"], "Entry": fmt_price(r["price"], r["pair"]),
                    "SL": fmt_price(r["sl"], r["pair"]), "TP1": fmt_price(r["tp1"], r["pair"]),
                    "R:R": rr_ratio(r), "Conf.": score_str,
                    "Trend": f"{trend_icon} {r['trend']}", "RSI": f"{r['rsi']:.1f}",
                    "Quality": r["quality"],
                })
    ...
    with tabs[2]:
        if not st.session_state.scan_results:
            ...
            with c2:
                ...
                sup_str = "<br>".join([f"• {fmt_price(s,pair)}" for s in struct["support"]]) or "—"
                res_str = "<br>".join([f"• {fmt_price(r,pair)}" for r in struct["resistance"]]) or "—"
                st.markdown(f"""
    ...
    with tabs[3]:
        ...
        if uploaded:
            ...
            with result_col:
                ...
                if sig is None:
                    st.markdown(f"""
                    <div style="background:#1a0000;border:1px solid #440000;border-radius:10px;padding:20px;
                                color:#FF3547;text-align:center;font-size:14px;">
                      🚫 NO TRADE<br>
                      <span style="color:#666;font-size:12px;margin-top:8px;display:block;">{notes}</span>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown("**Analysis Notes:**")
                    for note in notes:
                        st.markdown(f"<div style='color:#666;font-size:12px;margin:2px 0;'>→ {note}</div>",
                                    unsafe_allow_html=True)
                    st.markdown("---")
                    st.markdown(signal_card_html(sig), unsafe_allow_html=True)
                    st.plotly_chart(confidence_gauge(sig["score"]), use_container_width=True)
